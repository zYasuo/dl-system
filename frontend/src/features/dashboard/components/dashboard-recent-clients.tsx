"use client";

import Link from "next/link";
import { formatDistanceToNow } from "date-fns";
import { pt } from "date-fns/locale";
import { ChevronRight } from "lucide-react";
import { useDashboardRecentClients } from "@/features/dashboard/hooks/use-dashboard-recent-clients";
import { DashboardInputCard } from "@/features/dashboard/components/dashboard-input-card";
import { clientDocumentLabel } from "@/features/clients/lib/display";
import { cn } from "@/lib/utils";
import { buttonVariants } from "@/shared/components/ui/button-variants";
import { Separator } from "@/shared/components/ui/separator";
import { Skeleton } from "@/shared/components/ui/skeleton";

export function DashboardRecentClients() {
  const { data, isPending, isError } = useDashboardRecentClients();
  const clients = data?.data ?? [];

  return (
    <DashboardInputCard
      title="Clientes recentes"
      description="Actualizados há pouco tempo."
      className="min-h-[280px]"
      contentClassName="gap-0 px-0 sm:px-0"
    >
      {isError ? (
        <p className="px-4 text-sm text-muted-foreground">
          Não foi possível carregar a lista. Tenta a página{" "}
          <Link
            href="/dashboard/clients"
            className="text-primary underline-offset-4 hover:underline"
          >
            Clientes
          </Link>
          .
        </p>
      ) : isPending ? (
        <ul className="flex flex-col gap-0 px-4" aria-hidden>
          {Array.from({ length: 5 }).map((_, i) => (
            <li key={i} className="py-3">
              <Skeleton className="h-4 w-full max-w-xs" />
              <Skeleton className="mt-2 h-3 w-24" />
            </li>
          ))}
        </ul>
      ) : clients.length === 0 ? (
        <p className="px-4 text-sm text-muted-foreground">
          Ainda não há clientes registados.
        </p>
      ) : (
        <ul className="text-sm" aria-label="Lista de clientes recentes">
          {clients.map((client, index) => (
            <li key={client.id}>
              {index > 0 ? <Separator className="my-0" /> : null}
              <Link
                href={`/dashboard/clients/${encodeURIComponent(client.id)}`}
                className={cn(
                  "flex items-start gap-3 px-4 py-3 transition-colors",
                  "hover:bg-sidebar-accent/60 focus-visible:bg-sidebar-accent/60 focus-visible:outline-none",
                )}
              >
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-foreground">{client.name}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    {clientDocumentLabel(client) ?? "—"} · Atualizado{" "}
                    {formatDistanceToNow(new Date(client.updatedAt), {
                      addSuffix: true,
                      locale: pt,
                    })}
                  </p>
                </div>
                <ChevronRight
                  className="mt-1 size-4 shrink-0 text-muted-foreground"
                  aria-hidden
                />
              </Link>
            </li>
          ))}
        </ul>
      )}
      {!isPending && !isError && clients.length > 0 ? (
        <div className="mt-auto border-t border-border/80 bg-muted/20 px-3 py-3 sm:px-4">
          <Link
            href="/dashboard/clients"
            className={cn(
              buttonVariants({ variant: "ghost", size: "sm" }),
              "w-full justify-between text-muted-foreground hover:text-foreground",
            )}
          >
            Ver todos os clientes
            <ChevronRight className="size-4" aria-hidden />
          </Link>
        </div>
      ) : null}
    </DashboardInputCard>
  );
}
